...
#veriyi olustururken r dili once numeric yap??yo.
is.character(y)#true
is.factor(f) #true
is.logical(l) #true 
...
class(y) #bu da bi yonetem
class(x)
class(f)
class(l)

#class gelen sonuca hangi sinifta oldugunu yazar ve gonderir
#is.factor,is.character fln ise true false gonderir. bu yuzden bunlar daha cok ise yarica ileride
...
y <- as.factor(y) #faktore cevrdi, levels verdi 
class(y)
...
x2 <- c(11,23,45,0,-5)
x2l <- as.logical(x2)
x2l #sadece sifirn icin false verdi gerisi true
...
#factor to numeric
as.numeric(x) #1,2,3,4 verdi. ama de??erler a b c d idi??? nasil oldu??
#bunun sebebi levellard??r. levellar neyse onu verir

x1 <- factor(c('a','b','c','d','a','b'))
as.numeric(x1) #123412 seklinde verdi
x1 #levellar abcd verdi safece. levellar sadece 1 tane olur: o level??n icinde 
